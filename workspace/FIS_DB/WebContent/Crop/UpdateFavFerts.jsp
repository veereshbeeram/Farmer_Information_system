...
<html>
	...
	<head>
		<title>Farmer Buddy - Fertiliser Listing</title>
...
<link rel="stylesheet" type="text/css" href = "<s:url value = 'css/login.css' />" >
	...
	<p>	Crop Selected :<b> <s:property value="crop.name"/> </b></p>
		<table class="table">	
				<tr>
					<td style="font-size:larger">Delete Fertiliser</td>
				<td>
					<s:form action="deleteOldFavFert">
						<input type="hidden" name="crop.name" id="cropName" value="<s:property value="%{crop.name}"/>"/>
						<s:select list="crop.favFerts" name="crop.oldFavFert"></s:select>
				...
				<tr>
					<td style="font-size:larger">Add More Favourable Fertiliser</td>
					<td>
						<s:form action="addNewFavFert">
							<input type="hidden" name="crop.name" id="cropName" value="<s:property value="%{crop.name}"/>"/>
							<s:label labelposition="left"/>
							<br/>
							<table BORDER="2" BORDERCOLOR="#336699" CELLPADDING="1" CELLSPACING="5" WIDTH="100%">
								<s:select list="crop.unfavFerts" name="crop.newFavFert"/>
							</table>
							<s:submit value="Add"/>
